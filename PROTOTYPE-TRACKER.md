...
## Current Step: I-8
## Last Completed: I-6.1 (`/incentives` Restructure: 2 Tabs + Sticky Tier Section + Expanded Leaderboard + Wingz-mark TierBadge)
...
| I-6.1 | `/incentives` Restructure: 2 Tabs + Sticky Tier Section + Expanded Leaderboard + Wingz-mark TierBadge | ✅ Done    | **Tabs restructure:** `/incentives` reduced from 3 → 2 tabs ("Incentives" + "Leaderboard"). Tier Progress tab dropped; old `TierProgressTab` deleted. **Schema:** `LeaderboardEntry.county` added; `CurrentDriver.currentRank` + `totalDrivers` + `county` added. **Seed:** 20 LeaderboardEntries (was 10) sorted desc by $, all anonymized handles, all `isCurrentDriver: false`, all with Atlanta-metro counties (Fulton/DeKalb/Cobb/Gwinnett/Henry/Clayton). Current driver lives OUTSIDE the top 20 — `currentRank: 47, totalDrivers: 200, county: 'Fulton County', bonusesEarnedThisMonth: 80`. **TierBadge redesign:** medal/trophy lucide icons replaced with `<Image src="/WINGZLOGO2.png">` — green Wingz mark on tier-colored circle bg (Bronze #B45309 / Silver #94A3B8 / Gold #EAB308 / Platinum #7C3AED), same Wingz mark across all 4 tiers, only bg color varies. **New `TierProgressSection`** composite (`components/driver/tier-progress-section.tsx`) — sticky card-pill at top of Incentives tab. Reads `useVariants().tierProgress`. `tier-linear` = card with "Earned this month — May 2026" header + `$80` green hero + hairline divider + 4 Wingz-mark TierBadges + horizontal progress bar with $ callout above marker + tier $ thresholds + inline "You're in Silver — $70 to Gold" status copy. `tier-stack` = same hero + vertical stack of 4 tier rows inside the card (Platinum top → Bronze bottom): each row = TierBadge + label + threshold + status (Achieved/Current/`$X to go`/Locked); current Silver row highlighted with silver-soft bg + 2px silver border. **Leaderboard layout (both variants):** flex-column with sticky region (header + YourPlacementCard + Top 3) + scrollable region (ranks 4–20 + footer). **YourPlacementCard:** always-visible green-tinted card showing `#47 of 200 · $80` + Wingz-mark Silver TierBadge + "Silver · Fulton County". **list variant:** Top 3 rendered as 3 rows in a card (sticky); ranks 4–20 in a scrollable card below. **podium variant:** 1st = 144px gold-tinted card with 2px gold top border + md TierBadge; 2nd/3rd = 104px white cards with sm TierBadge; NO standalone rank number circle (position implies rank); handles wrap rather than truncate; county shown as small gray subtitle below handle. Both variants close with "Names anonymized for privacy." footer at the END of the scroll. **Variant Toggle:** Sections 4 + 5 unchanged (no rename, no new variants). Independent of Section 1 (no pill coupling); no month chevron; no real names; no points metric; no per-trip $ framing. |
