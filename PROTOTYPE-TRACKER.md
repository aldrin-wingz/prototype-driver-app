...
## Current Step
**App-I-5** (Dynamic Ends-in indicator on `<IncentiveCard>`; W2-1; independent — no Manager pair; Auto model). **App-I-4 ✅ approved 2026-05-12 LOCAL ONLY** — App caught up v3 → v6 in one prompt (Goal discriminated union + drop legacy `timeframe`/`enabled` + add `startDate`/`endDate` ISO strings + new `formatRollingWindow` + `computeCurrentWindowProgress` helpers + IncentiveCard mode-aware progress + **explicit rolling-window date chip** per user direction). **Corrected mid-stream per user feedback:** semantic shifted from "Best Y-day window so far" → "Current Y-day window" (driver-facing metric, not eval question). 6 files changed (1 schema bump + helpers + IncentiveCard + popover + 2 pre-existing stale-ref bug fixes for `.targetCount` → `.goal` + `.name` → `.title`). v0 backfill pending.
