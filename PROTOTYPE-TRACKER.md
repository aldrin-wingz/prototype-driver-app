...
**Tier-based $ model (post I-4.6 — REPLACES the points system from I-4.2).** Each `IncentiveDefinition` has a `tierLevel: 'gold' | 'silver' | 'bronze'`. Constant `INCENTIVE_TIER_BONUSES = { gold: 50, silver: 30, bronze: 10 }`. Every program's `bonusAmount` is locked to its tier value (gold programs = $50, silver = $30, bronze = $10). When a program completes the driver earns the tier-based dollar amount, full stop — no separate "points" abstraction. Leaderboard ranks drivers by `bonusesEarnedThisMonth` ($ accumulated this month). Driver's own tier badge (Bronze/Silver/Gold/Platinum) comes from `totalBonusesEarnedThisMonth` against `TierConfig.threshold` (Bronze $0 / Silver $50 / Gold $150 / Platinum $300). **Cadence:** Tier Progress + Leaderboard run on a MONTHLY cycle (e.g. "May 2026"); /payout runs on a WEEKLY pay-period cycle. Different fields, different concepts.

**No-flow / data-driven only.** This prototype renders STATES from seed. NO completion events, NO tier-up events, NO payout-fire events, NO month-rollover events. All states seeded directly. Achievement Unlock + Tier Unlock dialogs (I-8) are mock-triggered via [DEV] buttons.
...
| I-4.1 | Driver Incentives Hub Page (`/incentives`) + Dashboard Carousel Rework | ⬜ Planned | New stack-pushed `/incentives` page. Originally built with 3 tabs (Incentives / Leaderboard / Tier Progress placeholders); **restructured in I-6.1 to 2 tabs** (Incentives / Leaderboard) — Tier Progress tab dropped, tier section absorbed into the top of the Incentives tab as a sticky card-pill. Rework `dashboard-card-section` variant from 4 stacked cards → swipe carousel. Add "View All" link to all dashboard variants. **`UpcomingPayoutWidget` STAYS on dashboard** (does NOT move). |
...
| I-4.4 | `/payout` Period-Aware: 3 Tabs + Period Nav + Summary Tri-State + 2 Variants | ✅ Approved | (Post-refinement.) Adds **Rides Upcoming** tab (3rd, from My Rides accepted; drops on completion). Period selector with `<` `>` chevrons (2 prior + 1 future periods seeded). Tab rename "Completed Incentives" → **"Incentives"** (shows earned + in-progress for the period). Summary tri-state: **Earned + Upcoming + Incentives = Projected** hero ("Final" on closed periods). Single merged outer card with hero + hairline + 3 metric cells. Two layout variants behind toggle Section 3 "Payout Summary": **`boxed-tabs`** (default — pill-style cells INSIDE the card with standard ~16px page padding; active = full Wingz green bg + white text) / **`edge-to-edge-tabs`** (cells extend FLUSH to viewport horizontal edges; active = subtle green bg tint + green underline). Both drop the separate tab row + count subtitle (cells double as clickable tabs; counts live in list header). Sticky behavior: summary block stays put, only list scrolls. |
...
| I-4.6 | Tier System Rework — Drop Points, Tier-Based $ + Monthly Cadence | ⬜ Planned | (Added 2026-05-01 after I-4.4 approval. Reverses parts of I-4.2's points additions.) **Schema:** drop `INCENTIVE_TIER_POINTS`; add `INCENTIVE_TIER_BONUSES = { gold: 50, silver: 30, bronze: 10 }`. Drop `LeaderboardEntry.pointsEarnedThisPeriod`; rename `bonusesEarned` (existing secondary $) → `bonusesEarnedThisMonth` (now PRIMARY ranking metric — drop secondary-figure concept). Drop `CurrentDriver.pointsAccumulatedThisPeriod`; rename `totalBonusesEarnedThisPeriod` → `totalBonusesEarnedThisMonth`. `TierConfig.threshold` becomes $-based monthly: Bronze $0 / Silver $50 / Gold $150 / Platinum $300. **Seed:** lock all programs' `bonusAmount` to tier-based value (Weekend Warrior + Peak Performer = $50 gold; Early Bird = $30 silver; Loyalty Streak = $10 bronze — was $50/$100/$75/$85). Rewrite LeaderboardEntries with $ values for May 2026 (#1: $230 / #2: $180 / #3: $130 / #4 current: $80 / rest descending). CurrentDriver.totalBonusesEarnedThisMonth: 80 (Silver tier, $70 to Gold). Cascading visual updates auto-flow through banners, IncentiveCards, /payout summary. NO new UI in this step — schema/seed only. |
...
| I-6   | Tier Progress + Leaderboard — Combined Tab Content (`/incentives`) | ✅ Approved (v1) | (Combined step — absorbs former I-7. Both tabs filled in one v0 pass.) **Tier Progress tab — 2 variants (toggle Section 4 "Tier Progress"):** `tier-linear` (4 tier icons across the top + single progress bar with position marker + "You're in Silver — $X to Gold" copy below) / `tier-stack` (vertical stack of 4 tier cards Bronze/Silver/Gold/Platinum, each with threshold + status achieved ✓ / current ◉ / locked 🔒, current tier highlighted with tier color bg). **Leaderboard tab — 2 variants (toggle Section 5 "Leaderboard"):** `leaderboard-list` (plain ranked list, # + handle + $ earned + tier badge per row, current driver row highlighted) / `leaderboard-podium` (top 3 in podium layout — 1st centered taller, 2nd left, 3rd right; ranks 4+ as ranked list below; current driver row highlighted in the list). Both tabs use **monthly cadence** ("This month — May 2026"). Build shared `TierBadge` composite. **Note:** Restructured in I-6.1 (3 tabs → 2 tabs, Wingz-mark TierBadge redesign, expanded leaderboard with YourPlacementCard, podium cleanup). |
| I-6.1 | `/incentives` Restructure: 2 Tabs + Sticky Tier Section + Expanded Leaderboard + Wingz-mark TierBadge | ✅ Approved (v1) | (Polish/restructure step on top of I-6 v1. Added + approved 2026-05-01.) **Restructure `/incentives` from 3 tabs to 2 tabs** (drop Tier Progress tab; tier section moves to top of Incentives tab as sticky card-pill — same pattern as `/payout` summary). **Schema:** add `LeaderboardEntry.county`; add `CurrentDriver.currentRank` + `totalDrivers` + `county`. **Seed:** 20 LeaderboardEntries (was 10), all with county; current driver OUTSIDE top 20 — `currentRank: 47`, `totalDrivers: 200`, `county: 'Fulton County'`, `bonusesEarnedThisMonth: 80`. **Leaderboard layout:** sticky Header + sticky **YourPlacementCard** (always visible — shows #47 of 200 + $ + tier + county) + sticky Top 3 region (podium for podium variant; rows 1-3 for list variant) + scrollable ranks 4–20 (default visible 4–10, scroll reveals 11–20). Each row + podium card adds `county` (small gray subtitle). **TierBadge redesign:** all 4 tiers = green Wingz mark on a tier-colored circle bg (Bronze #B45309 / Silver #94A3B8 / Gold #EAB308 / Platinum #7C3AED) — replaces medal/shield/trophy icons; unifies brand identity with ride card `banner-wingz-hero`. **Podium cleanup:** drop standalone rank number circle (position implies rank); fix handle truncation. Variant Toggle Section 4 + 5 stay (no rename). Tier/leaderboard variants still stand alone. |
| I-6.2 | `/incentives` + Dashboard Polish: Usernames + Leaderboard $ Distribution + Gold-First Sort + Dashboard Earnings + Section Reorder | ⬜ Planned | (Polish step on top of I-6.1 v1. Added 2026-05-01 after I-6.1 v1 review.) **6 parts.** **Schema:** add `CurrentDriver.username: string` (editable display name, default seed `'alex_b'`). **Seed:** rewrite all 20 `LEADERBOARD_ENTRIES.handle` values to realistic anonymized usernames (e.g., `mike_atl`, `j_williams`, `sarah_d`, `raj_patel`, …) — no `Driver-####` pattern; rewrite `bonusesEarnedThisMonth` so all 20 rows > $80, monotonically descending (rank 1=$230, rank 2=$180, rank 3=$130, rank 4=$125, …, rank 20=$85). Tier per row stays derived from $ against `TierConfig.threshold`. **Sort rule:** render incentive program list sorted by `tierLevel` desc (Gold → Silver → Bronze) on Dashboard carousel + `/incentives` Incentives tab + `/payout` Incentives tab. Resulting order: Weekend Warrior → Peak Performer → Early Bird → Loyalty Streak. **Dashboard earnings:** seed `thisMonthEarnings: 907.02` (= 184.80 + 353.22 + 124 + 95 + 150 from 5 completed trips); `thisMonthTripsCount: 5`; seed `lastMonthEarnings: 725.62` (≈80% of this month). Wire chevron toggle on earnings card (local React state — `useState<'thisMonth' \| 'lastMonth'>`). **Dashboard section reorder (top → bottom):** UpcomingPayoutWidget → Driver Incentives surfacing → Earnings card (with chevron) → Confirm-Upcoming-Trip prompt → New Requests preview → Next Accepted Ride. Order is identical across all 3 dashboard variants. **YourPlacementCard:** render `username` + small pencil/edit icon → tap toggles inline input + Save/Cancel; save updates local state only (mock-only, no backend). Forward-only deltas on top of I-6.1 v1 build. **DO NOT revert v0.** |
...
|-----------|-------------|---------|
| Tabs | `@/components/ui/tabs` | I-0a (My Rides 3-tab row: In Progress / Needs Action / Upcoming), I-4.1 (`/incentives` 3-tab placeholder — restructured in I-6.1 to 2 tabs: Incentives / Leaderboard, Tier Progress tab dropped), I-4.3 (`/payout` 2-tab interface — superseded in I-4.4), I-4.4 (`/payout` 3-tab interface: Rides Completed / Rides Upcoming / Incentives — both layout variants) |
| Card | `@/components/ui/card` | I-0a, I-0b, I-3, I-4, I-6, I-7 |
| Button | `@/components/ui/button` | All steps |
| Avatar | `@/components/ui/avatar` | I-0a, I-6 (Tier Progress + Leaderboard combined; I-7 absorbed) |
| Badge | `@/components/ui/badge` | I-2 (IncentiveBadgeRenderer), I-3, I-4, I-5, I-6 (TierBadge — redesigned in I-6.1 to Wingz mark on tier-color bg) |
| Progress | `@/components/ui/progress` | I-3 (ProgressMeter), I-6 (tier progress) |
| ToggleGroup | `@/components/ui/toggle-group` | I-3 (period selector if used), I-5 (filter) |
...
| Table | `@/components/ui/table` | I-6 (leaderboard rows on tablet — combined step absorbs former I-7) |
| ScrollArea | `@/components/ui/scroll-area` | I-2 (multi-pill row), I-5 (filter chip row), I-6 (leaderboard scroll for ranks 4–20 in I-6.1) |
...
| VariantToggle | I-1 | All variant-set steps (I-2, I-3, I-4.4, **I-6**). Sheet has 5 sections post-I-6: 1 pill (Ride Card Indicator), 2 dashboard (Dashboard Incentives), 3 payoutSummary (Payout Summary — added I-4.4, variants `boxed-tabs` / `edge-to-edge-tabs`), 4 tierProgress (Tier Progress — added I-6, variants `tier-linear` / `tier-stack`), 5 leaderboard (Leaderboard — added I-6, variants `leaderboard-list` / `leaderboard-podium`). |
...
| IncentivesPage | I-4.1 (built) → I-6 (tabs filled) → I-6.1 (restructured to 2 tabs) | New `/incentives` route — Driver Incentives Hub. Composed from header (back chevron + "Driver Incentives" title + Variants pill) + Tabs row. **I-4.1**: 3-tab placeholder (Incentives default / Leaderboard / Tier Progress). **I-6 v1**: filled both Leaderboard + Tier Progress tabs (I-7 absorbed). **I-6.1**: restructured to 2 tabs (Incentives default with sticky `TierProgressSection` at top + scrollable IncentiveCard list / Leaderboard with sticky `YourPlacementCard` + sticky Top 3 + scrollable ranks 4–20). Tier Progress tab dropped in I-6.1. |
...
| TierBadge | I-6 (built) → I-6.1 (redesigned) | Originally built in I-6 with medal/shield/trophy icons + tier-colored accents. **Redesigned in I-6.1**: green Wingz mark (`/WINGZLOGO2.png`) on a tier-colored circle bg (Bronze #B45309 / Silver #94A3B8 / Gold #EAB308 / Platinum #7C3AED). Same Wingz mark across all 4 tiers — only bg color changes. Used in: TierProgressSection (Incentives tab top), Leaderboard rows + podium + YourPlacementCard, Dashboard tier badge surface. Unifies brand identity with ride card `banner-wingz-hero`. |
| TierProgressSection | I-6.1 | Sticky card-pill at the top of the Incentives tab (replaces the former Tier Progress tab from I-6). Reads `useVariants().tierProgress`: `tier-linear` (4 Wingz-mark tier icons across top + horizontal progress bar with position marker + "You're in Silver — $X to Gold" copy below — all wrapped in a single card-pill matching the `/payout` summary card pattern) or `tier-stack` (vertical stack of 4 tier cards with threshold + status indicators achieved/current/locked, current tier highlighted with tier-color bg). Header inside the card: "Earned this month — May 2026" + `$<totalBonusesEarnedThisMonth>` hero figure. Sticky behavior: card-pill stays put; incentive program list scrolls under. |
| IncentivesTab | I-6.1 | Restructured composite for the Incentives tab on `/incentives`. Layout: sticky `TierProgressSection` at top + scrollable list of `IncentiveCard` instances below. Replaces the original simple stacked list from I-4.1 (no tier section) and I-6 (3-tab structure). |
| YourPlacementCard | I-6.1 | Sticky composite at the very top of the Leaderboard tab. Shows current driver placement among ALL drivers: "You're #<currentRank> of <totalDrivers>" + `$<bonusesEarnedThisMonth>` + `<county>` + small Wingz-mark TierBadge for current tier. Always visible regardless of scroll. Standard horizontal page padding (not edge-to-edge). |
| LeaderboardTab | I-6 (built) → I-6.1 (expanded) | `/incentives` Leaderboard tab. Reads `useVariants().leaderboard`: `leaderboard-list` or `leaderboard-podium`. **I-6 v1:** 10 drivers, current driver in list at #4, top 3 visually distinct. **I-6.1 expansion:** 20 drivers (none is the current driver), `county` field per row, sticky `YourPlacementCard` at very top, sticky Top 3 region, scrollable ranks 4–20 (default visible 4–10, scroll reveals 11–20). Each row + podium card shows Wingz-mark `TierBadge` + handle + county subtitle + $. Anonymity footer at end of scroll. |
...
- **Home / Dashboard** (`/`) — replicate slots 01a/01b/01c. **Section order (post I-6.2):** UpcomingPayoutWidget (top — added I-3) → Driver Incentives surfacing (added I-3, carousel in I-4.1) → Earnings card (with `<` `>` chevron toggle for This Month / Last Month — derived from `CurrentDriver.thisMonthEarnings` / `lastMonthEarnings`) → Confirm Your Upcoming Trip prompt (green-tinted) → New Requests preview (using `RideCard`) → Next Accepted Ride section (using `RideCard`). **Pre-I-6.2 order** (still in v0's I-6.1 v1 build, before I-6.2 ships): Earnings → UpcomingPayout → Confirm → Driver Incentives → New Requests → Next Accepted.
...
### Step I-4.6: Tier System Rework — Drop Points, Tier-Based $ + Monthly Cadence

**Why this step exists:** I-4.2 introduced a points abstraction (`INCENTIVE_TIER_POINTS = { gold: 3, silver: 2, bronze: 1 }`) where tier was a non-monetary status driven by accumulated points. After review the user decided this added cognitive overhead without clear payoff — it's cleaner if the tier IS the dollar value. A gold program is worth $50, silver $30, bronze $10, full stop. The leaderboard shows total $ earned (a tangible, comparable figure across drivers) instead of an abstract point sum. Driver tier badges (Bronze/Silver/Gold/Platinum) progress on $ accumulated this MONTH (longer, more meaningful gamification cadence than the weekly /payout period). This step is the schema/seed rewrite that makes that swap. NO new UI in this step — purely schema and seed changes that cascade through existing surfaces.

**Goal:** Drop the points system. Lock program bonuses to tier values. Switch leaderboard + tier progress to a monthly $ cadence. Rewrite the seed accordingly.

**Reference screenshots:** None (schema/seed only).
...
```ts
// REMOVE
export const INCENTIVE_TIER_POINTS = { gold: 3, silver: 2, bronze: 1 } as const;

// ADD
export const INCENTIVE_TIER_BONUSES = { gold: 50, silver: 30, bronze: 10 } as const;

// IncentiveDefinition stays — bonusAmount per program, but seed enforces tier-based values
// (every gold program: bonusAmount = 50; silver: 30; bronze: 10)

// CHANGED: LeaderboardEntry simplified
export interface LeaderboardEntry {
  rank: number;
  handle: string;
  bonusesEarnedThisMonth: number;   // ← PRIMARY ranking metric (replaces both pointsEarnedThisPeriod + secondary bonusesEarned)
  isCurrentDriver: boolean;
  tier: Tier;
}
// Note: drop pointsEarnedThisPeriod field entirely. Drop the "secondary $ figure" concept — bonusesEarnedThisMonth is the ONLY $ shown per row.

// CHANGED: CurrentDriver
export interface CurrentDriver {
  id: string;
  displayName: string;
  initials: string;
  currentTier: Tier;
  // REMOVE: pointsAccumulatedThisPeriod
  totalBonusesEarnedThisMonth: number;   // ← was totalBonusesEarnedThisPeriod, now MONTHLY scope
}

// CHANGED: TierConfig.threshold semantics shift from points → monthly $
// Bronze: 0
// Silver: 50
// Gold: 150
// Platinum: 300
...
`PayoutPeriodSummary` stays unchanged — `/payout` continues using weekly pay-period scope. Different concept, different fields, no overlap with monthly leaderboard/tier scope.

---

#### Part 2 — Seed updates

**Lock all programs' `bonusAmount` to tier-based values:**

| Program | tierLevel | OLD bonusAmount | NEW bonusAmount |
|---|---|---|---|
| Weekend Warrior | gold | $50 | $50 (unchanged) |
| Peak Performer | gold | $100 | **$50** |
| Early Bird | silver | $75 | **$30** |
| Loyalty Streak | bronze | $85 | **$10** |

After this rewrite, EVERY surface showing program $ amount auto-pulls from seed and updates: ride card banners ("Earn $50"), IncentiveCards on dashboard / `/incentives` Incentives tab / `/payout` Incentives tab, contribution popover copy, `/payout` summary calculations.

**Rewrite `LeaderboardEntries` (10 rows) for May 2026 — sorted by `bonusesEarnedThisMonth` descending:**

| Rank | handle | bonusesEarnedThisMonth | tier | isCurrentDriver |
|---|---|---|---|---|
| 1 | (varied handles like `racer_42`, anonymized) | $230 | Gold | false |
| 2 | (anonymized) | $180 | Gold | false |
| 3 | (anonymized) | $130 | Silver | false |
| 4 | (current driver — handle = whatever was already seeded) | **$80** | **Silver** | **true** |
| 5 | (anonymized) | $60 | Silver | false |
| 6 | (anonymized) | $40 | Bronze | false |
| 7 | (anonymized) | $30 | Bronze | false |
| 8 | (anonymized) | $20 | Bronze | false |
| 9 | (anonymized) | $10 | Bronze | false |
| 10 | (anonymized) | $0 | Bronze | false |

**Update `CurrentDriver`:**
- `totalBonusesEarnedThisMonth: 80` (matches rank #4 in leaderboard)
- `currentTier: 'silver'` (since 80 ≥ 50 < 150)
- Drop `pointsAccumulatedThisPeriod` field entirely

**Update `TierConfig` thresholds (monthly $-based):**
- Bronze: 0
- Silver: 50
- Gold: 150
- Platinum: 300

The `multiplier` parking-lot field stays in seed but still NOT visualized.

**`DriverIncentiveProgress` records** — the `earnedThisPeriod` boolean stays; nothing schema-level needed there. But if any program was previously "earned" with a non-tier $ value it still counts (e.g., Loyalty Streak earned for the period at $10 bronze).

---

#### Part 3 — Cascading visual updates check

This is a SCHEMA + SEED ONLY step. No new components. But because the values feeding existing components changed, expect visual changes on ALL surfaces that show program $ or driver-level $:

- Ride card banners (`banner-wingz-hero`, `achievement-banner`): "Earn $X" updates per program (e.g., Weekend Warrior $50, Peak Performer $50, Early Bird $30, Loyalty Streak $10).
- IncentiveCard on Dashboard carousel + `/incentives` Incentives tab + `/payout` Incentives tab: same.
- ProgramContributionIndicator popover: "Earn $X when complete" + "$Y added to next payout" reflect new amounts.
- `/payout` summary: Incentives mini-card $ figure recomputes (was $150 from $50+$100; now $100 from $50+$50 if both programs are in-progress, or different totals depending on completion mix).
- `/incentives` Incentives tab card list: same $ figures update.
- Dashboard tier badge (if rendered) reads `currentTier` → Silver per new seed.

These updates flow automatically from the seed change — no component code touched in this step.

---

**Constraints:**

- DO NOT add or modify any components (TierProgress / Leaderboard tabs are placeholder shells until I-6 fills them).
- DO NOT touch /payout summary math — it correctly sums program $ via existing logic; the new tier-based amounts flow through naturally.
- DO NOT introduce flow / events. All resets, accumulations, etc. remain seed-only.
- DO NOT add a "month period selector" with chevrons — single current-month view only ("May 2026"); backward/forward nav is a /payout concept.
- DO NOT keep `INCENTIVE_TIER_POINTS` or `pointsEarnedThisPeriod` or `pointsAccumulatedThisPeriod` as deprecated fields — REMOVE entirely.
- DO NOT preserve the "secondary $ figure on leaderboard rows" concept from old I-7 — there's only ONE figure per row now (`bonusesEarnedThisMonth`).

**Test Flows After This Step:**

- [ ] `INCENTIVE_TIER_POINTS` removed from `lib/data/incentives.ts`
- [ ] `INCENTIVE_TIER_BONUSES` exported with values { gold: 50, silver: 30, bronze: 10 }
- [ ] All 4 programs have `bonusAmount` matching their `tierLevel` (Weekend Warrior $50, Peak Performer $50, Early Bird $30, Loyalty Streak $10)
- [ ] Ride card banners on Requests + My Rides reflect new $ amounts (Weekend Warrior $50, Peak Performer $50)
- [ ] IncentiveCard list on `/incentives` Incentives tab shows new $ amounts
- [ ] `/payout` summary Incentives mini-card recomputes (e.g., $80 if 1 silver completed + 1 gold projected — exact value depends on seed mix)
- [ ] `LeaderboardEntry` schema: `bonusesEarnedThisMonth` field present; `pointsEarnedThisPeriod` and old `bonusesEarned` fields REMOVED
- [ ] LeaderboardEntries sorted desc by `bonusesEarnedThisMonth` with values matching the May 2026 distribution above
- [ ] `CurrentDriver.totalBonusesEarnedThisMonth: 80`, `currentTier: 'silver'`; `pointsAccumulatedThisPeriod` REMOVED
- [ ] `TierConfig.threshold` values: Bronze 0 / Silver 50 / Gold 150 / Platinum 300
- [ ] No TypeScript errors after the schema rewrite (placeholder tabs may need a quick adapter if they referenced removed fields)
- [ ] Mobile portrait 375×812 viewport, no horizontal overflow
- [ ] Zero TypeScript errors

**Backend Implications (capture only):**

- Production: backend computes `bonusesEarnedThisMonth` per driver per month. Frontend reads.
- Tier classification: backend-derived from `totalBonusesEarnedThisMonth` against `TierConfig.threshold`.
- Monthly rollover events live on backend (not in this prototype).
- Production may keep program-specific bonusAmounts (not tier-locked) for flexibility — prototype simplifies for clarity. Tier dollar values can be defaults that programs override.
...
### Step I-6: Tier Progress + Leaderboard — Combined Tab Content (`/incentives`)

**Why this step exists:** Both `/incentives` Tier Progress and Leaderboard tabs are still placeholder shells from I-4.1. After I-4.6 the schema/seed are correct ($-based monthly cadence, leaderboard sorted by `bonusesEarnedThisMonth`, driver tier from `totalBonusesEarnedThisMonth`). I-6 fills BOTH tabs in one v0 pass with variant sets — gives stakeholders side-by-side comparison of tier visualization styles AND leaderboard layouts in one review. Common gamification patterns kept simple and mobile-tight.

**Goal:** Fill the Tier Progress and Leaderboard tabs on `/incentives` simultaneously. Each tab gets a 2-variant set behind a new Variant Toggle Sheet section. Build the shared `TierBadge` composite once and reuse across both tabs (and the dashboard tier badge surface from I-3).

**Reference screenshots:** None (both tabs are net-new surfaces).

**This step has SIX parts in one v0 generation pass.**

> **Forward-looking note:** This step ships v1 with the structure described below (3 tabs on `/incentives`; medal/shield/trophy TierBadge icons; 10-driver leaderboard with current driver in top 10). **I-6.1 (post-build polish/restructure)** drops the Tier Progress tab (tier section moves to top of Incentives tab as sticky card-pill), redesigns TierBadge with Wingz-mark-on-tier-bg, expands the leaderboard to 20 drivers with county field, and adds a sticky `YourPlacementCard` for the case when the current driver is outside the top 20. Build I-6 normally with the 6 parts below; I-6.1 handles the restructure on top.

---

#### Part 1 — Variant set additions in `lib/variants.ts`

```ts
// NEW
export type TierProgressVariant = 'tier-linear' | 'tier-stack';
export type LeaderboardVariant = 'leaderboard-list' | 'leaderboard-podium';

// CHANGED: VariantSelection gains 2 new keys
export interface VariantSelection {
  pill: PillVariant;
  dashboard: DashboardVariant;
  payoutSummary: PayoutSummaryVariant;
  tierProgress: TierProgressVariant;     // NEW
  leaderboard: LeaderboardVariant;       // NEW
}
...
export const DEFAULT_VARIANTS: VariantSelection = {
  ...
  tierProgress: 'tier-linear',           // default
  leaderboard: 'leaderboard-list',       // default
};

export const VARIANT_LABELS = {
  pill: { /* unchanged */ },
  dashboard: { /* unchanged */ },
  payoutSummary: { /* unchanged */ },
  tierProgress: {
...
URL param keys: `?tierProgress=…` and `?leaderboard=…`. localStorage keys follow existing pattern.

**Variant Toggle Sheet** gains TWO new sections (4 + 5):

- **Section 4 — "Tier Progress"** with sub-copy "How tier progression is visualized on the Tier Progress tab."
  - "Linear Progress" — `tier-linear`
  - "Tier Stack" — `tier-stack`
- **Section 5 — "Leaderboard"** with sub-copy "How drivers are ranked on the Leaderboard tab."
  - "Ranked List" — `leaderboard-list`
  - "Podium Top 3" — `leaderboard-podium`

Identical chrome to Sections 1–3.

---

#### Part 2 — Shared `TierBadge` composite

Build once, reuse across both tabs + dashboard tier badge.

- Composed from `Badge` + small icon/medal + tier label
- Props: `tier: 'bronze' | 'silver' | 'gold' | 'platinum'`, `size: 'sm' | 'md' | 'lg'`, optional `showLabel: boolean`
- Tier color tokens:
  - Bronze: `#B45309`
  - Silver: `#94A3B8`
  - Gold: `#EAB308`
  - Platinum: deep purple `#7C3AED` (or similar — pick what reads cleanly alongside the other 3)
- Icon: simple medal/shield outline; color matches tier
- Used in: TierProgressTab (both variants), LeaderboardTab (both variants — small size per row), Dashboard tier badge surface (existing from I-3)

> **Note (post-build):** TierBadge gets redesigned in **I-6.1** to use a green Wingz mark on a tier-colored circle bg (replaces medals/shields/trophy). Build I-6 with the simple medal/shield/trophy approach; I-6.1 swaps in the Wingz-mark visual.

---

#### Part 3 — Tier Progress tab — `tier-linear` variant

Replace the Tier Progress tab placeholder.

Layout:
```
┌───────────────────────────────────┐
│  Earned this month — May 2026     │
│  $80                              │  ← big green hero ($totalBonusesEarnedThisMonth)
│                                   │
│   🥉    🥈    🥇    🏆            │  ← 4 small TierBadges, evenly spaced across
│  ─────●─────────────────────      │  ← horizontal progress bar with position marker
│                                   │  ← marker at 80/300 = ~27% along the bar
│  You're in Silver — $70 to Gold   │  ← copy: current tier + threshold delta
└───────────────────────────────────┘
```

Specifics:
- Big hero: `$<totalBonusesEarnedThisMonth>` from `CurrentDriver`. Green primary color.
- 4 tier icons (small `TierBadge` instances, ~24px each) evenly spaced across full available width
- Horizontal progress bar: 0% at Bronze ($0) → 100% at Platinum ($300). Position marker at `totalBonusesEarnedThisMonth` / 300 × 100%
- Marker: filled green dot, slightly larger than the bar, centered on the bar line
- Copy below: "You're in {currentTier} — ${nextThreshold - totalBonusesEarnedThisMonth} to {nextTier}"
- If driver is at Platinum: "You've reached the highest tier — Platinum!" instead

---

#### Part 4 — Tier Progress tab — `tier-stack` variant
...
│  $80                              │
│                                   │
│  🏆 Platinum   $300+   🔒 Locked  │  ← stacked tier cards
│  🥇 Gold       $150+   🔒 $70 to go│
│  🥈 Silver     $50+    ◉ Current  │  ← highlighted with silver bg
│  🥉 Bronze     $0+     ✓ Achieved │
└───────────────────────────────────┘
```

Specifics:
- Same hero header as Variant A
- Vertical stack of 4 cards (top-down: Platinum → Gold → Silver → Bronze, OR Bronze → Platinum bottom-up — pick what reads as "ladder up")
- Each card: TierBadge icon + label on left, threshold $ in middle, status indicator on right
- Status indicators:
  - Tiers above current: 🔒 Locked (or `🔒 $X to go` for the immediate next tier)
  - Current tier: ◉ Current (with tier-color bg tint on the card itself)
  - Tiers below current: ✓ Achieved
- Current tier card: bg color = the tier's color (silver → silver bg with white/dark text per contrast); other cards: white bg

---

#### Part 5 — Leaderboard tab — `leaderboard-list` variant

Replace the Leaderboard tab placeholder.
...
│  2   🥇  trip_titan       $180     │
│  3   🥈  mile_master      $130     │
│  4   🥈  YOU              $80      │  ← highlighted row (green bg tint)
│  5   🥈  dawn_driver      $60      │
│  6   🥉  swift_sam        $40      │
│  7   🥉  road_ace         $30      │
│  8   🥉  lane_legend      $20      │
...
Specifics:
- Header: "This month — May 2026" — small gray, no chevrons (single-month view)
- Each row: rank # + small `TierBadge` + handle + `$<bonusesEarnedThisMonth>` (right-aligned, green)
- Current driver row (`isCurrentDriver: true`): highlighted with green-50 bg tint + slightly bolder text + handle reads "YOU" (or the seeded handle)
- No top-3 special treatment in this variant (keep it flat / minimal)
- Anonymity footer at bottom: "Names anonymized for privacy."

---

#### Part 6 — Leaderboard tab — `leaderboard-podium` variant
...
│         │  1   │                   │
│  ┌──────┤ 🥇   ├──────┐            │
│  │  2   │racer42│  3  │            │  ← podium for top 3
│  │ 🥇   │ $230 │ 🥈   │
│  │titan │      │master│
│  │ $180 │      │ $130 │
│  └──────┴──────┴──────┘
│  ──────── ranks 4+ ─────────       │
│  4   🥈  YOU              $80      │  ← highlighted (green bg tint)
│  5   🥈  dawn_driver      $60      │
│  6   🥉  swift_sam        $40      │
│  7+ ...                            │
└────────────────────────────────────┘
...
Specifics:
- Same header as Variant A
- Top 3 podium: 1st in middle (taller, ~120px), 2nd left (~96px), 3rd right (~96px). Each podium card shows: position number, TierBadge, handle, $.
- Below podium: ranks 4+ as a flat ranked list (same layout as `leaderboard-list` rows)
- Current driver row in the list highlighted same way (green-50 bg)
- Anonymity footer at bottom

---

**Constraints:**

- DO NOT couple tier/leaderboard variants to Section 1 (pill variant) — they are independent. No `useVariants().pill` reads in TierProgressTab or LeaderboardTab.
- DO NOT add a month chevron / period selector — single current-month view only ("May 2026" is hardcoded display from a `CURRENT_MONTH_LABEL` constant or similar simple seed).
- DO NOT add real driver names — handles only (e.g. `racer_42`, `trip_titan`).
- DO NOT show secondary $ figures or any "points" metric anywhere — purely $ on leaderboard rows.
- DO NOT add tier-up event logic — display only; Tier Unlock dialog comes in I-8.
- DO NOT change the placeholder shells before this step — they get replaced wholesale here.

**Test Flows After This Step:**

- [ ] Variant Toggle Sheet shows 5 sections; Section 4 "Tier Progress" with 2 radio options; Section 5 "Leaderboard" with 2 radio options
- [ ] URL param `?tierProgress=tier-stack` initializes Variant B on Tier Progress tab
- [ ] URL param `?leaderboard=leaderboard-podium` initializes Variant B on Leaderboard tab
- [ ] localStorage persists both selections across reload
- [ ] `tier-linear`: hero $ + 4 tier icons + progress bar + position marker + threshold copy ("$70 to Gold")
- [ ] `tier-stack`: 4 tier cards stacked, current tier (Silver) highlighted with tier-color bg, status indicators correct (achieved/current/locked + "to go" copy on next-up tier)
- [ ] `leaderboard-list`: 10 rows in rank order, current driver row highlighted, all rows show rank + TierBadge + handle + $ figure
- [ ] `leaderboard-podium`: top 3 in podium, ranks 4+ as list, current driver highlighted in the list
- [ ] `TierBadge` composite renders all 4 tiers correctly with the right colors
- [ ] Anonymity footer present on Leaderboard tab in both variants
- [ ] "May 2026" displayed as static label on both tabs
- [ ] No tier-up animations / celebrations (those are I-8)
- [ ] Mobile portrait 375×812 viewport, no horizontal overflow on either variant
- [ ] Zero TypeScript errors

**Backend Implications (capture only):**

- Production: backend computes `bonusesEarnedThisMonth` per driver per month. Frontend reads.
- Driver tier classification = backend-derived from `totalBonusesEarnedThisMonth` against `TierConfig.threshold`.
- Leaderboard rank = sorted server-side; frontend reads ordered list.
- Anonymized handles = production privacy requirement; backend assigns/derives.
- Variant selection is client UI state — no backend impact.

---

### Step I-6.1: `/incentives` Restructure — 2 Tabs + Sticky Tier Section + Expanded Leaderboard + Wingz-mark TierBadge

**Why this step exists:** I-6 v1 shipped with 3 tabs (Incentives / Leaderboard / Tier Progress) + medal/shield/trophy TierBadge icons + a 10-driver leaderboard with the current driver in the top 10. After review the user decided several things together: (1) Tier visualization shouldn't have its own tab — better as a sticky context section at the top of the Incentives tab so drivers see their tier alongside the programs they're working on. (2) The leaderboard should expand to 20 drivers with county info per row, default visible 10 with scroll-to-reveal-20. (3) The current driver should be **outside** the top 20 (at #47 of 200) with a sticky "Your Placement" card at the very top of the Leaderboard tab. (4) TierBadge should be redesigned to use the green Wingz mark on a tier-colored bg — replaces medals/shields/trophy and unifies brand identity with the ride card `banner-wingz-hero` variant. (5) Podium needs cleanup — drop the standalone rank number circle (position implies rank), fix handle truncation. This step delivers all five together as deltas on top of I-6 v1. **DO NOT revert v0 to before I-6 — apply this as forward-only changes.**

**Goal:** Restructure `/incentives` from 3 tabs to 2 tabs. Move tier visualization to the top of the Incentives tab as a sticky card-pill. Redesign TierBadge with Wingz-mark-on-tier-bg. Expand the Leaderboard tab with `YourPlacementCard` + 20-driver seed + county field + sticky regions. Clean up the podium.

**Reference screenshots:** None (deltas on top of I-6 v1 build).
...
```ts
// CHANGED: LeaderboardEntry gains county
export interface LeaderboardEntry {
  rank: number;
  handle: string;
  bonusesEarnedThisMonth: number;
  isCurrentDriver: boolean;       // unused in seed (current driver outside top 20) — keep for future
  tier: Tier;
  county: string;                 // NEW
}

// CHANGED: CurrentDriver gains rank position fields + county
export interface CurrentDriver {
  id: string;
  displayName: string;
  initials: string;
  currentTier: Tier;
  totalBonusesEarnedThisMonth: number;   // existing
  currentRank: number;                   // NEW: e.g., 47
  totalDrivers: number;                  // NEW: e.g., 200
  county: string;                        // NEW: e.g., 'Fulton County'
}
```
...
**`LEADERBOARD_ENTRIES` array — 20 drivers** (was 10), all anonymized handles, sorted desc by `bonusesEarnedThisMonth`. Each row gets a `county` field. None of the 20 entries is the current driver (`isCurrentDriver: false` for all 20).

Sample distribution (use realistic Atlanta-metro county names):

| Rank | handle | $ | tier | county |
|---|---|---|---|---|
| 1 | (anon) | $230 | gold | Fulton County |
| 2 | (anon) | $180 | gold | DeKalb County |
| 3 | (anon) | $130 | silver | Cobb County |
| 4 | (anon) | $80 | silver | Fulton County |
| 5 | (anon) | $60 | silver | Gwinnett County |
| 6 | (anon) | $40 | bronze | Henry County |
| 7 | (anon) | $30 | bronze | Clayton County |
| 8 | (anon) | $20 | bronze | DeKalb County |
| 9 | (anon) | $10 | bronze | Fulton County |
| 10 | (anon) | $0 | bronze | Cobb County |
| 11–20 | (anon) | $0 | bronze | (vary across the same counties) |

(Lower-rank rows can all be $0 or trail off — the gradient just needs to feel realistic for v1 review.)

**`CurrentDriver` updates:**
- `totalBonusesEarnedThisMonth: 80` (existing — unchanged)
- `currentRank: 47` (NEW)
- `totalDrivers: 200` (NEW)
- `county: 'Fulton County'` (NEW)
- `currentTier: 'silver'` (existing — unchanged)

---

#### Part 3 — Tab restructure: 3 tabs → 2 tabs on `/incentives`

Restructure `IncentivesPage` (built in I-4.1) from 3 tabs to 2 tabs:

- **Tab 1 — "Incentives"** (default) — sticky `TierProgressSection` at top + scrollable `IncentiveCard` list below
- **Tab 2 — "Leaderboard"** — sticky Header + sticky `YourPlacementCard` + sticky Top 3 region + scrollable ranks 4–20

**Drop the old "Tier Progress" tab entirely.** Drop the `TierProgressTab` composite from I-6 (or rename + relocate it as `TierProgressSection` per Part 5).

Variant Toggle Sheet Section 4 ("Tier Progress") **stays** (same label, same options) — it now controls the tier visualization at the top of the Incentives tab instead of a dedicated tab.

---

#### Part 4 — `TierBadge` redesign: Wingz mark on tier-color bg

Replace the medal/shield/trophy icon mix from I-6 v1 with a unified Wingz-mark-on-tier-bg pattern.

- **Foreground:** the existing green Wingz mark image (`/WINGZLOGO2.png`) — same brand recall element as ride card `banner-wingz-hero`
- **Background:** tier-colored circle (the only thing that varies between tiers):
  - Bronze: `#B45309`
  - Silver: `#94A3B8`
  - Gold: `#EAB308`
  - Platinum: `#7C3AED`
- Same Wingz mark for all 4 tiers; only bg color changes
- Apply EVERYWHERE TierBadge is rendered: TierProgressSection (Incentives tab top), Leaderboard rows, podium cards, YourPlacementCard, dashboard tier badge surface

This unifies the prototype's visual identity around the green Wingz mark — same element appears on ride card banners, IncentiveCards, dashboard, and now tier visualizations.

---

#### Part 5 — Incentives tab: `TierProgressSection` (sticky top, both variants)

Move the I-6 TierProgressTab content into a new sticky composite at the top of the Incentives tab.

Sticky card-pill at the top of the Incentives tab. Reads `useVariants().tierProgress`.

**Shared header (both variants):**
- Inside the card-pill, top section: "Earned this month — May 2026" (small uppercase gray) + `$<totalBonusesEarnedThisMonth>` hero (large green)
- Hairline divider below the hero

**Variant `tier-linear` (default):**
```
╔═════════════════════════════════╗
║  Earned this month — May 2026   ║
║  $80                            ║
║  ─── hairline ───                ║
║  ⓦ    ⓦ    ⓦ    ⓦ              ║  ← 4 Wingz-mark TierBadges (Bronze/Silver/Gold/Platinum bg)
║  ──●──────────                   ║  ← horizontal progress bar with green dot marker (~27%)
║  You're in Silver — $70 to Gold ║  ← inline copy (NOT a separate card)
╚═════════════════════════════════╝
```
...
║  ⓦ Platinum   $300+   🔒 Locked ║
║  ⓦ Gold       $150+   🔒 $70 to go│
║  ⓦ Silver     $50+    ◉ Current ║  ← silver bg, bolder
║  ⓦ Bronze     $0+     ✓ Achieved║
╚═════════════════════════════════╝
```

**Sticky behavior:**
- The card-pill stays pinned at the top of the Incentives tab content
- Below it, the IncentiveCard list scrolls independently
- Same pattern as `/payout` summary card sticky behavior

**Below the sticky `TierProgressSection`:**
- Scrollable list of `IncentiveCard` instances (existing visual from I-3, themed by `useVariants().pill`)
- Same content as the original Incentives tab from I-4.1 — no change to the program list itself

---

#### Part 6 — Leaderboard tab: expanded layout with sticky regions

Restructure the Leaderboard tab. Three vertical regions:

**Region 1 (sticky) — Header**
- "This month — May 2026" — small gray, no chevrons

**Region 2 (sticky) — `YourPlacementCard`**
- Always visible. Pinned just below the header.
- Layout (single card, same horizontal padding as the rest of the page):
  ```
  ┌─────────────────────────────────┐
  │  Your placement                  │
  │  #47 of 200 · $80                │  ← bold $ in green
  │  ⓦ Silver · Fulton County        │  ← Wingz-mark TierBadge + tier name + county
  └─────────────────────────────────┘
  ```
- Reads from `CurrentDriver`: `currentRank`, `totalDrivers`, `bonusesEarnedThisMonth`, `currentTier`, `county`
- Always shown (even if a future seed places the current driver inside top 20 — for prototype, current driver is at #47, outside top 20)
- Standard horizontal page padding (NOT edge-to-edge)

**Region 3 (mixed) — Top 3 (sticky) + Ranks 4–20 (scrollable)**

For `leaderboard-list` variant:
- Top 3 rows (ranks 1, 2, 3) are visually grouped at the top of the rankings list and STICKY (stay pinned when user scrolls within the rankings)
- Subtle visual separation between Top 3 and ranks 4+
- Ranks 4–20 scroll within their region (default visible: 4–10; user scrolls to reveal 11–20)
- Each row: rank # + small `TierBadge` (Wingz mark on tier bg) + handle + county (small gray subtitle below handle, OR inline) + `$<bonusesEarnedThisMonth>` (right-aligned)
- Anonymity footer at the END of the scroll: "Names anonymized for privacy."

For `leaderboard-podium` variant:
- Top 3 podium pinned/sticky (same podium pattern: 1st centered taller ~140px, 2nd left ~96px, 3rd right ~96px)
- Each podium card: large Wingz-mark TierBadge + handle + county (small gray below handle) + `$<bonusesEarnedThisMonth>`
- **NO standalone rank number circle** on podium cards — position implies rank (center = 1st, left = 2nd, right = 3rd)
- 1st card has subtle gold-tinted bg or top border for visual prominence
- Below podium: thin divider with "RANKS 4+" label
- Below divider: ranks 4–20 scroll within their region (default visible: 4–10; user scrolls to reveal 11–20)
- Each rank row: same as in `leaderboard-list` variant
- Handle truncation: ellipsis or smaller font; never overflow podium card width
- Anonymity footer at the END of the scroll

**Sticky scroll behavior — both variants:**
- Header + YourPlacementCard + Top 3 region all stay pinned
- Only the ranks 4–20 list scrolls (within its allotted vertical space)
- When the user scrolls down within ranks, the sticky regions remain visible

---

**Constraints:**

- DO NOT keep a 3rd "Tier Progress" tab — explicitly drop it from `IncentivesPage`. The Tier Progress tab placeholder shell from I-4.1 + the I-6 TierProgressTab composite should be removed in this step.
- DO NOT revert v0 to before I-6 — apply this as forward-only changes on top of the existing build.
- DO NOT couple tier/leaderboard variants to Section 1 (pill variant). No `useVariants().pill` reads in TierProgressSection or LeaderboardTab.
- DO NOT add a month chevron / period selector — single current-month view only ("May 2026" hardcoded).
- DO NOT add real driver names — handles only.
- DO NOT show secondary $ figures or any "points" metric anywhere — purely $ on leaderboard rows.
- DO NOT add tier-up event logic — display only; Tier Unlock dialog comes in I-8.
- DO NOT use medal/shield/trophy icons in TierBadge — Wingz mark on tier-color bg is the only visual.
- DO NOT show standalone rank number circles on podium cards (position implies rank).
- DO NOT extend the leaderboard edge-to-edge horizontally — standard page padding stays.
- DO NOT include the current driver in the 20 LeaderboardEntries — they live in `YourPlacementCard` only.
- DO NOT rename Variant Toggle Sheet Section 4 ("Tier Progress" stays, even though it's no longer its own tab).

**Test Flows After This Step:**

- [ ] `/incentives` page has exactly 2 tabs: Incentives / Leaderboard (no Tier Progress tab)
- [ ] Variant Toggle Sheet still shows 5 sections; Section 4 + Section 5 unchanged labels
- [ ] **Incentives tab — `tier-linear`**: TierProgressSection renders as a sticky card-pill at the top with hero $ + 4 Wingz-mark tier icons + progress bar + position marker + inline threshold copy "$70 to Gold"
- [ ] **Incentives tab — `tier-stack`**: TierProgressSection renders 4 tier cards stacked, current Silver highlighted, status indicators correct
- [ ] **Incentives tab — sticky behavior**: scrolling the IncentiveCard list keeps TierProgressSection pinned at top
- [ ] **Leaderboard tab — header + YourPlacementCard + Top 3** all sticky/pinned at top; only ranks 4–20 scroll
- [ ] **YourPlacementCard**: shows "#47 of 200" + "$80" + Silver Wingz-mark TierBadge + "Fulton County"
- [ ] `leaderboard-list`: ranks 1–3 sticky group, ranks 4–20 scrollable, default visible 4–10, scroll reveals 11–20
- [ ] `leaderboard-podium`: top 3 podium sticky, NO standalone rank circles, ranks 4–20 scrollable
- [ ] Each rank row + podium card shows handle + county + $ + Wingz-mark TierBadge
- [ ] Podium handle truncation handled (ellipsis or smaller font, no overflow)
- [ ] **TierBadge composite renders Wingz mark on tier-color bg for all 4 tiers** (no medals/shields/trophies anywhere)
- [ ] TierBadge appears identically across TierProgressSection, podium, rank rows, YourPlacementCard, dashboard tier badge surface
- [ ] Anonymity footer at end of leaderboard scroll
- [ ] Current driver NOT in the 20 LeaderboardEntries
- [ ] Mobile portrait 375×812 viewport, no horizontal overflow on any variant
- [ ] Zero TypeScript errors

**Backend Implications (capture only):**

- Production: backend computes `bonusesEarnedThisMonth` per driver per month + per-driver rank against the global driver pool. Frontend reads precomputed values.
- `currentRank` + `totalDrivers` are backend-derived per period.
- `county` lives on driver records in production; backend includes it on leaderboard responses.
- Sticky scroll regions are pure UI state — no backend impact.
