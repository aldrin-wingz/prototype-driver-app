...
## 2026-05-15 Post-Lock Drift Patches (Read Before PRD)

App side stayed quiescent post-App-MVP-2 ✅ (2026-05-14). Two small drift patches landed 2026-05-15 LOCAL ONLY to keep cross-prototype parity with Manager-side polish:

1. **Canonical purple recolor** — all 8 App seed incentives recolored to `#8B5CF6` via batch perl. Matches Manager's `INCENTIVE_PILL_COLOR` export. ColorPicker was admin-side only and is now removed entirely — brand color is enforced everywhere.
2. **Pause filter** — `getAllIncentiveProgress` filters out `enabled === false` incentives so drivers don't see paused programs. (Manager re-introduced `enabled` as a pause overlay 2026-05-15; this is the App-side honoring of that flag.)

All other 2026-05-15 waves (POLISH / ORDER / FILTER / RESUME / FILTER-UNIFY / GUARD + bug fixes) are Manager-only and do not change the App's driver-facing UX. App MVP queue remains LOCKED at 10/10 ✅. PRD scaffolding via `prd-finalizer` will produce `PRD - v1 - Driver App.md` from the post-MVP-2 stripped surface (header card + Completed rides only; plain outline button; no Appeals subsystem; pause-filter on dashboard).
...
| **App-I-6** | **✅ Approved 2026-05-12 LOCAL ONLY — Resume Wave Per-Incentive History (Counted ⏐ Missed Out tabs) + Disqualified-Trip UI.** Cross-prototype lockstep with Manager P-12 ✅ complete. Landed: NEW `lib/data/disqualifications.ts` (App-side mirror of P-12 types + storage keys + 10 seeded disqualifications all for currentDriver 'driver-7821' + 6 seeded appeals + `seedTriggeringTripSnapshots`); NEW `lib/data/disqualification-helpers.ts` (pure helpers + `useDisqualificationsState` hook + `buildCountedTripRows`/`buildMissedOutRows`); NEW `app/incentives/[id]/history/page.tsx` route + NEW `components/driver/incentive-history-view.tsx` view (header card + tab strip + Counted simple-list with Restored badge + Missed Out desaturated cards with amber Disqualified badge + reason line + inline expandable driver/manager text); EDIT `lib/data/incentive-utils.ts` (added `incentiveId` to `IncentiveProgressInfo`); EDIT `components/driver/dashboard-incentive-section.tsx` (outer `<button>` → `<div role="button">` to allow nested `<Link>`; added "View history →" affordance with double stopPropagation). Approved appeals → trip moves to Counted (Restored badge); pending → blue "Appeal under review" badge; denied → red badge + tap-to-expand manager reason. No-appeal → placeholder alert (App-I-7 wires `<DisputeAppealSheet>`). 7 files changed (4 new + 3 edits). v0 backfill pending. Higher-thinking model. |
| **App-I-7** | **✅ Approved 2026-05-13 LOCAL ONLY — Resume Wave Dispute Form + Appeal Feedback + 4-wave App-I-7.1 polish (folded into approval).** Bundles W3-A3 + W3-A4. **Base App-I-7 (6 files, 3 new + 3 edits):** NEW `lib/data/appeal-acks.ts` (localStorage helpers at `wingz-incentives:appeal-acks:v1`; append-only `string[]`; defensive parse; idempotent); NEW `components/driver/dispute-appeal-sheet.tsx` (shadcn `Sheet` bottom-side; criterion-level red-tinted banner; create-mode `<Textarea>` reason min-10-chars + optional comments + Cancel/Submit; parent owns hook + passes `onSubmit(driverText)` so card state flips immediately); REFINE `components/driver/appeal-result-dialog.tsx` (later DELETED in polish wave 1); EDIT rides view + dashboard host. **App-I-7.1 polish (4 waves):** **(W1) Discoverability + naming + sheet unification** — promoted "Your Rides" button → status-tone+icon (amber AlertTriangle/sky Clock/red XCircle) when action-class status; renamed `<CriterionCard>` muted pill "Appeal pending" → "Appeal submitted"; new 5th `IncentiveStatus 'appeal-under-review'` split from `temp-disqualified` (pending-only criteria no longer surface "Action needed"); precedence in `getIncentiveStatus`: denied → no-appeal → pending → all-✓; unified `<DisputeAppealSheet>` to 4 modes (`create` / `view-pending` / `view-approved` / `view-denied`) sharing same Sheet shell + criterion banner + Your appeal section, Manager response section differentiates won (emerald) vs lost (amber); **DELETED standalone `<AppealResultDialog>`** (folded into the Sheet); `<AppealReentryPopupHost>` relocated into `dispute-appeal-sheet.tsx`; `<CriterionCard>` consolidated to single `onOpenAppealSheet(mode, state)` callback. **(W2) "Your Rides" → "Completed Rides"** dashboard button + aria-label; section header inside `<IncentiveRidesView>` renamed "Your rides" → "Completed rides"; **removed aggregate `<IncentiveStatusPill>` from per-incentive header** (criterion-card row carries appeal state per rule). **(W3) Inside `<IncentiveRidesView>` header mirrors outside `<IncentiveCard>` verbatim** — title pill + ends-in indicator share row 1 (urgent amber `Hourglass` / muted "Ends MMM DD"); description; mode-aware `<ProgressMeter>` imported from `dashboard-incentive-section.tsx`; rolling-window mode embeds "X of Y in last Z days" caption; **dropped full date-range subtitle + bordered green Window chip**. **(W4) "Appeal submitted" pill** restyled gray → sky-blue (`border-sky-300 bg-sky-50 text-sky-700` + `Clock text-sky-600`) so the criterion-level pill matches the dashboard status pill's visual vocabulary. **`<DisqualifiedBanner>` status-aware via new `status` prop with 3 variants** — amber `AlertTriangle` "Temporarily disqualified — appeal a criterion below to request a review. Trips continue to count toward your goal." (any ✗ no-appeal) · sky `Clock` "Appeal under review — your appeal is awaiting manager review. Trips continue to count toward your goal." (only ✗ pending) · red `XCircle` "Appeal denied — the disqualification stands for this campaign." (any ✗ denied). `<IncentiveRidesView>` derives `bannerStatus` from `criteriaStates` with denied > no-appeal > pending precedence and passes it down. **No schema change.** Pairs with Manager P-13b ✅. Higher-thinking model. **Strategy-impacting ripple:** new `IncentiveStatus 'appeal-under-review'` enters the cross-prototype status vocabulary (App-side only today; Manager P-14 NEW may want to surface "submitted vs unresolved appeals" axis when reviewing incentives at scale). v0 backfill pending. |
...
### MVP Scope-Down Wave (queued 2026-05-14 post-I-7 ✅ Manager-side; v1 Manager queue REOPENED per user direction)

User received post-I-7 feedback and wants to scope DOWN to a tighter v1 MVP without Appeals subsystem. "With-Appeals" version preserved at sibling-folder archives `v1/Incentives V1 CS Tool — Archive (with Appeals)/` + `v1/Incentives V1 App — Archive (with Appeals)/`. **3 decisions locked 2026-05-14:** (a) archive = sibling folders ✅ DONE; (b) App stops surfacing `driverTargeting` entirely (driver targeting becomes Manager-side admin metadata only, parallels marketScope/clientScope per P-10 precedent); (c) all driver-targeting criteria evaluate AS OF `incentive.startDate` (NOT today). NO schema bump in the wave. After MVP-6 ✅ → v1 MVP Manager queue LOCKED again + move to PRD / story drafting. **App side of wave = only App-MVP-2** (cross-prototype pair of Manager MVP-1); MVP-3..MVP-6 are Manager-only.

| # | Description |
|---|------|
| **App-MVP-2** | **✅ Approved 2026-05-14 LOCAL ONLY — MVP Scope-Down Wave: Appeals Surgical Strip (App) + Re-vendor Manager Preview** (Auto model; cross-prototype pair of Manager MVP-1; execution-only). **Phase A — App-side strip (6 deletes + 7 edits):** DELETED `lib/data/appeal-acks.ts` · `lib/data/disqualifications.ts` · `lib/data/disqualification-helpers.ts` · `components/driver/criterion-card.tsx` · `components/driver/dispute-appeal-sheet.tsx` · `components/driver/incentive-status-pill.tsx`. EDITED `app/page.tsx` (dropped `AppealReentryPopupHost` import + mount); `components/driver/incentive-rides-view.tsx` (major rewrite — replaced `buildCountedTripRows` with inline `seedTrips` filter; dropped Eligibility section, CriterionCard list, DisqualifiedBanner, DisputeAppealSheet state machine, all appeal state derivations; view = header card + Completed rides only); `components/driver/dashboard-incentive-section.tsx` (dropped `useDisqualificationsState`/`getIncentiveStatus`/`IncentiveStatusPill`/status-tone Completed Rides styling/AlertTriangle+Clock+XCircle icons; button = plain outline); `app/incentives/page.tsx` (dropped `useDisqualificationsState`/`getIncentiveStatus` hooks; derived status from `progress.isComplete` only); `lib/data/past-outcomes.ts` (dropped `"missed-criterion"` from `PastOutcomeType` union + dropped inc-past-004 appeal-denied seed); `lib/data/incentives.ts` (dropped `driverTargeting?` field + all `driverTargeting:[...]` seed array literals); `lib/data/incentive-utils.ts` NO-EDIT. **Phase B — Manager preview re-vendor (Manager working dir):** DELETED 6 vendored files under `v1/Incentives V1 CS Tool/components/preview/vendored/` + `_lib/`; re-vendored 5 components + 4 helpers from post-strip App canonical with `Source last synced: 2026-05-14`; Manager preview view consumers needed no edits. **Acceptance gate met:** grep both prototypes case-insensitive `appeal|disqualif` → zero active-code Appeals hits (remaining: MVP-1 strip docblocks + `match-drivers.ts:20` P-12 hook comment + unrelated `lib/agent-mock/drivers-data.ts` onboarding string). App `/incentives/[id]/rides` = header card + Completed rides only; App dashboard button = plain outline; Manager `/incentives/[id]` renders 3 tabs only; embedded driver-app preview pane post-strip. Schema untouched (`:v3` stays). Both dev servers HTTP 200. **App side of MVP wave COMPLETE** — lockstep returns to Manager-only for MVP-3..MVP-6. v0 backfill pending. |
...
### Disqualification + Appeal Reads (App-I-6 ✅ LANDED 2026-05-12 + App-I-7 from W3-S)

App-I-6 ✅ landed 2026-05-12 LOCAL ONLY — pairs with Manager P-12. App-side mirror lives at `lib/data/disqualifications.ts` (types + storage keys + 10-disqualification / 6-appeal seed all remapped to `currentDriver.id` 'driver-7821') and `lib/data/disqualification-helpers.ts` (pure helpers + `useDisqualificationsState` persistence hook + view-model helpers `buildCountedTripRows` / `buildMissedOutRows`).

Reads:
- `wingz-incentives:disqualifications:v6` — driver-side, filtered via `getDisqualificationsFor(disqualifications, driverId, incentiveId)`.
- `wingz-incentives:appeals:v6` — driver-side, joined via `getAppealForDisqualification(appeals, disqualificationId)`.
- Per-incentive Counted tab list = (a) `seedTrips.filter(t => t.status==='completed' && t.incentiveTypes.includes(incentiveType))` + (b) approved-appeal trips restored from their `triggeringTripId` snapshots.
- Per-incentive Missed Out tab list = `getDisqualificationsFor(...).filter(d => !appealForD?.status === 'approved')` (approved appeals move to Counted per Phase B sign-off).

Writes (App-I-7 will wire from `<DisputeAppealSheet>`):
- `createAppeal({ driverId, incentiveId, disqualificationId, driverText })` via the `useDisqualificationsState` hook — generates id + createdAt + `status: 'pending'`; persists to `:appeals:v6`. App-I-6 stubs the tap-to-dispute affordance behind a placeholder `alert()` — App-I-7 replaces with the bottom-sheet form.

Acknowledgement state (App-I-7):
- Reserved key `wingz-incentives:appeal-acks:v1` (declared but unused in App-I-6) will hold the set of `appealId`s the driver has acknowledged so `<AppealResultDialog>` re-entry doesn't re-show.
