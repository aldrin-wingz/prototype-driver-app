...
**Tier-based $ model (post I-4.6 — REPLACES the points system from I-4.2).** Each `IncentiveDefinition` carries a `tierLevel: 'gold' | 'silver' | 'bronze'`. **Tier IS the dollar value:** gold programs pay $50 when complete, silver $30, bronze $10 (`INCENTIVE_TIER_BONUSES = { gold: 50, silver: 30, bronze: 10 }`). Leaderboard ranks drivers by `bonusesEarnedThisMonth` ($ accumulated this month) — single $ per row, no secondary metric, no points. Driver's own tier badge (Bronze/Silver/Gold/Platinum) comes from `totalBonusesEarnedThisMonth` against `TierConfig.threshold` (Bronze $0 / Silver $50 / Gold $150 / Platinum $300, all monthly). NO `INCENTIVE_TIER_POINTS`, no `pointsEarnedThisPeriod`, no `pointsAccumulatedThisPeriod` fields anywhere — those were dropped in I-4.6. **Cadence:** Tier Progress + Leaderboard run on a MONTHLY cycle ("This month — May 2026"); /payout runs on a WEEKLY pay-period cycle. Different fields, different concepts.
...
**Section order (post I-6.2):**

1. **UpcomingPayoutWidget** (top — added I-3, promoted to top in I-6.2) — bold `$amount` + base/bonus split + "Next payout: <date>" caption; tap → `/payout`.
2. **Driver Incentives surfacing** (added I-3, promoted to position 2 in I-6.2) — variant-controlled (`dashboard-banner` / `dashboard-card-section` (carousel) / `dashboard-widget-integrated`). **Programs sorted Gold → Silver → Bronze** (post I-6.2). "View All" → `/incentives` (added I-4.1).
3. **Earnings card** — period label (`This Month` / `Last Month`) toggled by `<` `>` chevrons flanking the amount; large bold black `$X.XX`; sub-label `EARNINGS` (small caps gray); 3-column stat row (`Trips` / `On-Time Performance` / `Send Backs`). **Seed (post I-6.2):** `thisMonthEarnings: 907.02` (sum of 5 completed trips: 184.80+353.22+124+95+150), `thisMonthTripsCount: 5`, `lastMonthEarnings: 725.62` (≈80% of this month). Chevrons toggle local React state.
4. **"Confirm Your Upcoming Trip" prompt** (green-tinted card, `~#10B981` light wash) — green checkmark icon + body text.
5. **"New Requests" section header + `View All` link** + ride card preview using the shared `RideCard` component.
6. (slot 01c) **"Next Accepted Ride" section header** + ride card with the same anatomy plus a red `Not Confirmed` pill.

**Pre-I-6.2 order (still in v0's I-6.1 v1 build, before I-6.2 polish ships):** Earnings → UpcomingPayoutWidget → Confirm Upcoming Trip → Driver Incentives → New Requests → Next Accepted Ride. Documented for reference; the I-6.2 polish prompt instructs v0 to apply the new order.

Incentive surfacing variants in I-3 layer at position 2 in the section stack (post I-6.2). Do not push other existing sections off-screen.
...
- **IncentivesPage** — built in I-4.1 as the dedicated `/incentives` route. Stack-pushed (no bottom nav). Composed from a top header (back chevron + "Driver Incentives" title + Variants pill) + a `Tabs` row. **Originally 3 tabs in I-4.1 / I-6 v1** (Incentives default / Leaderboard / Tier Progress placeholders, then filled by combined I-6). **Restructured in I-6.1 to 2 tabs**: **Incentives** (default — sticky `TierProgressSection` at top + scrollable `IncentiveCard` list below) and **Leaderboard** (sticky header + sticky `YourPlacementCard` + sticky Top 3 region + scrollable ranks 4–20). Each `IncentiveCard` tap deep-links to filtered Requests, same contract as dashboard cards. **Does NOT contain `UpcomingPayoutWidget`** — that lives on the dashboard + `/payout` page. Read-only — no payout-action buttons.
...
1. ~~Standalone "Incentives Catalog" page~~ — **REINSTATED 2026-05-01.** TWO dedicated pages are now in scope (both stack-pushed, no bottom nav): (a) `/incentives` (added in I-4.1, restructured in I-6.1) — Driver Incentives Hub. **Originally 3 tabs in I-4.1 / I-6 v1 (Incentives default / Leaderboard / Tier Progress); restructured in I-6.1 to 2 tabs** — Incentives (default — sticky `TierProgressSection` at top + scrollable IncentiveCard list below) and Leaderboard (sticky `YourPlacementCard` + sticky Top 3 + scrollable ranks 4–20 with county per row). Reached via "View All" links on every dashboard incentive surfacing variant. (b) `/payout` (added in I-4.3, extended in I-4.4) — period-aware Upcoming Payout breakdown. Header has `<` `>` period selector. Summary section has 2 layout variants (Mini-Cards Below default / Tabs as Metrics) selectable via Variant Toggle Sheet Section 3. Three tabs: Rides Completed / Rides Upcoming / Incentives. Reached by tapping the dashboard `UpcomingPayoutWidget` (Sheet popup retired). Cards on either page still deep-link to filtered Requests with `?incentive=<type>` (same contract as dashboard cards).
...
| I-4.1 | **Driver Incentives Hub Page (`/incentives`) + Dashboard Carousel Rework.** Add new stack-pushed `/incentives` page with **3 tabs** (originally): Incentives (default), Leaderboard (placeholder), Tier Progress (placeholder — both filled by combined I-6, then Tier Progress tab dropped in I-6.1 — page is 2 tabs post-I-6.1). Rework `dashboard-card-section` variant: 4 stacked cards → swipe carousel (one card at a time, page dots, "View All" link top-right opens `/incentives`). Add "View All" link to all dashboard variants. **`UpcomingPayoutWidget` STAYS on dashboard** (does NOT move to `/incentives`). |
