...
| I-6 | **Tier Progress + Leaderboard — Combined Tab Content** (single combined step — absorbs former I-7). Both `/incentives` tabs filled in one v0 pass. **Tier Progress 2 variants (Section 4):** `tier-linear` (default — 4 tier icons + horizontal progress bar with position marker + "$X to next tier" copy) / `tier-stack` (4 tier cards stacked, achieved/current/locked status, current tier highlighted with tier-color bg). **Leaderboard 2 variants (Section 5):** `leaderboard-list` (default — plain ranked list with # + handle + $ + tier badge per row, current driver highlighted) / `leaderboard-podium` (top 3 in podium + ranks 4+ as list). Both tabs use **monthly cadence** ("This month — May 2026") — no chevron nav. Build shared `TierBadge` composite (used in both tabs + dashboard tier badge). Variants stand alone (no Section 1 coupling). |
